...
export const PinPerspective = ({ title }: { title?: string }) => {
  return (
    <motion.div className="pointer-events-none bg-transparent w-[30em] h-[20em] flex items-center justify-center opacity-0 group-hover/pin:opacity-100 z-[60] transition duration-500">
      {title && (
        <div className="absolute top-0 inset-x-0 flex justify-center">
          <div className="relative flex space-x-2 items-center z-10 rounded-full py-0.5 px-4 ring-1 ring-white/10">
            <span className=" font-roboto tracking-widest relative z-20 text-white text-xs font-bold inline-block py-0.5">
              {title}
            </span>
      ...
      ></motion.div>
      <>
          <motion.div className="absolute right-1/2 bottom-1/2 bg-gradient-to-b from-transparent bg-yellow-400 translate-y-[14px] w-px h-20 group-hover/pin:h-40 blur-[2px]" />
          <motion.div className="absolute right-1/2 bottom-1/2 bg-gradient-to-b from-transparent bg-primary-yellow translate-y-[14px] w-px h-20 group-hover/pin:h-40  " />
